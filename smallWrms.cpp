/*
c++ smallWrms.cpp -o smallWrms sims/util.o sims/geom.o -lm

./smallWrms move.cas stay.cas sapcut

Bvalue on CAs in move.cas and stay.cas used as a weight
full structure in move.pdb then rotated using CA superposition
...
float	rms121 ( Prot*, Prot*, Vec&, Vec&, Mat* );
...
void getpdb ( Prot* chain, FILE *pdb, float sapcut ) { 
...
float	acc[5555];
		...
		if (strncmp(line,"ATOM ",5) && strncmp(line,"HETATM ",7)) continue; // MSE is a HETATM
		// allow any atom if (strncmp(line+13,"CA ",3)) continue;
		sscanf(line,"%30c %f%f%f%f%f", junk, &x, &y, &z, &s, &t);
		if (t < NOISE) continue; // only read atoms with B-values (used as super wt.)
	...
	chain->len = n-1;
	Pi(chain->len) NL
	if (chain->len < 3) { Pi(chain->len) Pexit(Too few atoms: reduce sapcut) }
...
Prot	*prot1 = new Prot;
Prot	*prot2 = new Prot;
FILE	*one = fopen(argv[1],"r"); // move
FILE	*two = fopen(argv[2],"r"); // stay
	sscanf(argv[3],"%f", &sapcut);
	Pr(sapcut) NL
	getpdb(prot1,one,sapcut);
	fclose(one);
	getpdb(prot2,two,sapcut);
	fclose(two);
	rms = rms121(prot1,prot2,Va,Vb,&Rab);
	one = fopen(argv[4],"r");
	two = fopen("rot.pdb","w");
	// reread all atoms from protein one
	LOOP { Vec a,b; float x,y,z; int io;
		io = read_line(one,line);
		if (io<0) break;
		if (!strncmp(line,"END",3)) break;
		...
		b += Vb;
		line[30] = (char)0;
		fprintf(two,"%s %7.3f %7.3f %7.3f  1.00  0.00\n", line,b.x,b.y,b.z);
	}
}

float rms121 ( Prot *a, Prot *b, Vec &vecA,  Vec &vecB, Mat *R )
{
float	rms;
double  **rot;
double	*ww, *ac, *bc;
double	**va, **vb;
int	i, j, len = a->len;
	if (a->len != b->len) Pexit(Proteins have unequal weighted lengths)
	...
	FOR(i,len) ww[i] = a->acc[i-1]*b->acc[i-1];
	rms = supermac(ww,va,vb,len,ac,bc,rot);
	if (rms>99.999) { Pt(error) Pr(rms) NL exit(1); }
	vecA = Vec(ac[0],ac[1],ac[2]);
	vecB = Vec(bc[0],bc[1],bc[2]);
	R->A = Vec(rot[0][0],rot[0][1],rot[0][2]); 
	R->B = Vec(rot[1][0],rot[1][1],rot[1][2]); 
	R->C = Vec(rot[2][0],rot[2][1],rot[2][2]); 
/*
	R->A = Vec(rot[0][0],rot[1][0],rot[2][0]); 
	R->B = Vec(rot[0][1],rot[1][1],rot[2][1]); 
	R->C = Vec(rot[0][2],rot[1][2],rot[2][2]); 
*/
Pm((*R)) NL
	Pr(rms) Pi(len) NL
	return rms;
