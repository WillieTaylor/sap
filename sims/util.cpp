...
int sort3min ( float a, float b, float c ) {
	if ( a<b && a<c ) return 1;
	if ( b<a && b<c ) return 2;
	if ( c<a && c<b ) return 3;
	return 0;
...
int sort3max ( float a, float b, float c ) {
	if ( a>b && a>c ) return 1;
	if ( b>a && b>c ) return 2;
	if ( c>a && c>b ) return 3;
	return 0;
...
int sort3min ( int a, int b, int c ) {
	if ( a<b && a<c ) return 1;
	if ( b<a && b<c ) return 2;
	if ( c<a && c<b ) return 3;
	return 0;
...
int sort3max ( int a, int b, int c ) {
	if ( a>b && a>c ) return 1;
	if ( b>a && b>c ) return 2;
	if ( c>a && c>b ) return 3;
	return 0;
...
float pairup ( float** m, int n, Pairs *pairs ) 
// good heuristic method for NxN matrices (need not be symmetric)
{
Pairs	*pair = new Pairs[n*n];
int	*pick = new int[n];
int	*best = new int[n];
float	score = -99999;
int	k = 0;
	FOR (i,n) FOR(j,n) { 
		pair[k].a = i; pair[k].b = j; pair[k].c = ' '; pair[k].s = -m[i][j]; k++;
	} 
	//Pi(k) NL
	sort(pair,k); // sort on -ve value then negate to get big on top
	FOR(i,k) pair[i].s *= -1;
	FOR(i,k-n) { int got; float sum; // loop over starting pairs
		...
		FOR(j,k) { int ok = 1; // loop over list of pairs
			FOR(in,got) { // check new entry against list
		...
		FOR(in,got) { int p = pick[in];
			// Pi(in) Pi(p) Pi(pair[p].a) Pi(pair[p].b) Pr(pair[p].s) NL
			sum += pair[p].s;
		}
		// Pr(sum) NL
	...
	FOR(i,n) { int p = best[i];
		// Pi(pair[p].a) Pi(pair[p].b) Pr(pair[p].s) NL
		pairs[i].a = pair[p].a;
		pairs[i].b = pair[p].b;
		pairs[i].s = pair[p].s;
	}
...
float marryup ( float **mat, int n0, int n1, Pairs *pairs, int *n)
// takes the score matrix <mat> (n0xn1, high=good) and returns <n> matched pairs
// NB: returns a consistent pairing but not necessarily the highest scoring one.
{
int i,j;
float   *ranks, score;
int     **rank0, **rank1;
...
/*
        for (i=0; i<n0; i++) { for (j=0; j<n1; j++) printf("%7.1f", mat[i][j]); NL } NL
...
// stable marriage algorithm (not used outside here)
{
