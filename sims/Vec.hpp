/****NB****
	for vectors A and B and scalar c:
		A|B-c == A|(B-c) != (A|B)-c, dito for & and -
**********/ 
...
public:
	...
	bool	vec_in_seg  ( const Seg&, const float ) const; 	// TRUE if c lies over the line segment with margin
	bool	vec_in_seg  ( const Vec&, const Vec&, const float ) const; 	// dito
	...
	Vec	vec_on_seg  ( const Seg& ) const; 	// returns the image of Vec on a line segment (or endpoint)
