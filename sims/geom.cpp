...
float tri ( Vec a, Vec b, Vec c ) { return (a^b)*c; }
float triprod ( Vec a, Vec b, Vec c ) { return (a^b)*c; }
float trinorm ( Vec a, Vec b, Vec c ) {
	return (a.getNorm()^b.getNorm())*c.getNorm();
}
...
bool Seg::vec_in_seg ( const Vec &c, const float e ) const {
// TRUE if c lies over the line segment (with margin e)
float	d = dist_to_cut(A,B,c);
	if ( d<0.0+e || d>1.0-e ) return 0; else return 1;
}
bool Vec::vec_in_seg ( const Seg &s, const float e ) const {
// TRUE if c lies over the line segment (with margin e)
float	d = dist_to_cut(s.A,s.B,*this);
	if ( d<0.0+e || d>1.0-e ) return 0; else return 1;
}
bool Vec::vec_in_seg ( const Vec &a, const Vec &b, const float e ) const {
	return this->vec_in_seg(Seg(a,b),e);
}
...
Vec Vec::vec_on_seg ( const Seg &s ) const { // was dot2line()
// returns the image of c on line segment or end-point if closer
Vec	p = s.B-s.A, q = (*this)-s.A;
float	d = (p*q)/p.sqr();
Vec	g = s.A + p*d;
	if ((*this).vec_in_seg(s)) return g;
	// else return the closer end
	q = *this;
	if ((q|s.A) < (q|s.B)) return s.A; else return s.B;
}
...
int	p;
	if ((a.A|b.A)<NOISE || (a.A|b.B)<NOISE) return 0.0;
	if ((a.B|b.A)<NOISE || (a.B|b.B)<NOISE) return 0.0;
	p = parallel(x,y);
	if (p > 0) z = getz(a.A,a.B,b.A,b.B);
	if (p < 0) z = getz(a.A,a.B,b.B,b.A);
	...
	// start with the closest ends
	dmin = fmin(fmin(a.A|b.A,a.A|b.B),fmin(a.B|b.A,a.B|b.B));
	// find the closest end ([ab].[AB] over a line [ab]
	d = a.vec_to_line(b.A);
        if (d<dmin && a.vec_in_seg(b.A)) dmin = d;
        d = a.vec_to_line(b.B);
        if (d<dmin && a.vec_in_seg(b.B)) dmin = d;
        d = b.vec_to_line(a.A);
        if (d<dmin && b.vec_in_seg(a.A)) dmin = d;
        d = b.vec_to_line(a.B);
        if (d<dmin && b.vec_in_seg(a.B)) dmin = d;
	return dmin;
}
...
Vec	x = a-b, y = c-b;
	...
	if (d < NOISE-1) return PI;
	if (d > 1-NOISE) return 0;
	return acos(d);
}
...
float	vol, cos, sin, tor;
	...
	vol = tri(p,q,y);
	sin = r.len();
	// sign so left-hand is -ve
	if (vol < 0.0) sin = -sin;
	tor = angle1pi(sin,cos);
	return tor;
}

float torsion ( Seg ab, Seg cd ) {
	return torsion(ab.A,ab.B,cd.A,cd.B);
}

float dihedral ( Vec a, Vec b, Vec c, Vec d ) {
// returns the dihedral angle (torsion down contact normal)
	if (parallel(a-b,c-d)) return 0.0;
Seg	norm = line_on_line(Seg(a,b),Seg(c,d));
float	t = torsion(a,norm.A,norm.B,c);
	// set range -90..90 (change sign to keep hand)
	if (t >  halfPI) t -= PI; 
	if (t < -halfPI) t += PI;
	return t;
}

float dihedral ( Seg ab, Seg cd ) {
	return dihedral(ab.A,ab.B,cd.A,cd.B);
}
...
float lineOline ( Seg a, Seg b, Vec *box ) {
	lineOline(a.A,a.B, b.A,b.B, box);
}
...
int i, key[4], ley[4];
int parr=0, perp=0;
	// make input lines parallel
	if ((a-b)*(c-d) > 0) {
        	box[0] = a; box[1] = b; box[2] = c; box[3] = d;
	} else {
        	box[0] = b; box[1] = a; box[2] = c; box[3] = d;
	}
	a = box[0]; b = box[1]; c = box[2]; d = box[3];
        if (parallel(Seg(a,b),Seg(c,d)) != 0) { int in = 0;
                // parallel or anti
                ...
                Seg gh = line_on_line(Seg(a,b),Seg(c,d));
                g = gh.A; h = gh.B;
        }
        ga = g|a; gb = g|b; hc = h|c; hd = h|d;
	// get the longest g-h...ends midline
	x = g&h;
	y = (a&c)-x;
	z = (a&d)-x; if (z.len() > y.len()) y=z;
	z = (b&c)-x; if (z.len() > y.len()) y=z;
	z = (b&d)-x; if (z.len() > y.len()) y=z;
	if ((g|h) < NOISE) { // coplanar
		// get side of g=h point (by dot prod.)
		z = y-x;
		if ((a-x)*z < 0.0) ga = -ga;
		if ((b-x)*z < 0.0) gb = -gb;
		if ((c-x)*z < 0.0) hc = -hc;
		...
		// get side of xy plane
		x = g-h;
		z = x^y;
		// sign dist by side (+/- is arbitrary)
		if (tri(x,z,a-g)<0.0) ga = -ga;
		if (tri(x,z,b-g)<0.0) gb = -gb;
		if (tri(x,z,c-h)<0.0) hc = -hc;
		if (tri(x,z,d-h)<0.0) hd = -hd;
	}
        // dists now signed relative to the contact normal gh
        t[0] = ga; t[1] = gb; t[2] = hc; t[3] = hd;
        sort(t,key,4);
        for (i=0; i<4; i++) if (key[i]<2) ley[i] = 0; else ley[i] = 1;
        // ley < 2 = a or b; else c or d
        if (ley[0]==ley[1]) return 0.0; // no overlap
        lap = fabs(t[key[1]]-t[key[2]]);
        if (box)
        { int ke1 = key[1], ke2 = key[2],
              le1 = ley[1], le2 = ley[2];
          Vec tmp1, tmp2, tmp;
                if (ga > gb) x = box[0]-box[1];
                        else x = box[1]-box[0];
                if (hc > hd) y = box[2]-box[3];
                        else y = box[3]-box[2];
                x.setNorm(); y.setNorm();
                tmp1 = box[ke1];
                tmp2 = box[ke2];
                ...
                if (le1==le2) { // contained 
                        box[1] = tmp2;
                        if (le2==0) { box[3] = h + y*t[ke2]; } else { box[3] = g + x*t[ke2]; }
                } else { // staggered
                        box[3] = tmp2;
                        if (le2==0) { box[1] = h + y*t[ke2]; } else { box[1] = g + x*t[ke2]; }
                }
                if (le1==0) { box[2] = h + y*t[ke1]; } else { box[2] = g + x*t[ke1]; }
        }
        if ((box[0]-box[1])*(box[2]-box[3]) < 0) // swap to make edges parallel
        { Vec	tmp1 = box[0], tmp2 = box[1];
                box[0] = tmp2; box[1] = tmp1;
        }
	ab = box[0] | box[1];
	cd = box[2] | box[3];
        if (fabs(ab-cd) > 0.001) { float gh;
		ac = box[0] | box[2];
		bd = box[1] | box[3];
                printf("*NB* box has diff edges: ab=%f, cd=%f, ac=%f, bd=%f\n", ab,cd,ac,bd);
                Pv(box[0]) NL Pv(box[1]) NL Pv(box[2]) NL Pv(box[3]) NL Pv(h) NL Pv(g) NL
		Patom(21,box[0]) Patom(22,box[1]) Patom(23,box[2]) Patom(24,box[3]) Patom(25,h) Patom(26,g)
                ab = box[0]|box[1]; cd = box[2]|box[3];
                ac = box[0]|box[2]; bd = box[1]|box[3];
                gh = g|h;
                printf("     end-end distances : ab=%f, cd=%f, ac=%f, bd=%f, gh=%f\n", ab,cd,ac,bd,gh);
                printf("     end-box distances : ga=%f, gb=%f, hc=%f, hd=%f\n", ga,gb,hc,hd);
                ga = box[0]|g; gb = box[1]|g;
                hc = box[0]|h; hd = box[3]|h;
                printf("     end-box distances : ga=%f, gb=%f, hc=%f, hd=%f\n", ga,gb,hc,hd);
                if(perp) printf("perpendicular\n");
                if(parr) printf("parallel\n");
		if ((g|h)<NOISE) printf("coplanar\n");
                KILL
        }
        return lap;
}
...
int i, key[4], ley[4];
int parr=0, perp=0;
	aox[0] = a; aox[1] = b; aox[2] = c; aox[3] = d; 
	box[0] = a; box[1] = b; box[2] = c; box[3] = d; 
	x = (b-a).getNorm();
	y = (d-c).getNorm();
	z = (x^y).getNorm();
	...
	if (a*b<0.0) gb = -gb;
	if (a*d<0.0) hd = -hd;
	// dists now signed relative to the contact normal gh: right+, left-
	t[0] = ga; t[1] = gb; t[2] = hc; t[3] = hd; 
	sort(t,key,4);
	for (i=0; i<4; i++) if (key[i]<2) ley[i] = 0; else ley[i] = 1;
	// ley < 2 = a or b; else c or d
	...
	if (ley[1] != 0) {
        ...
        if (fabs(ab-cd) > 0.0001) { float gh;
                ...
                ab = aox[0]|aox[1]; cd = aox[2]|aox[3];
                ac = aox[0]|aox[2]; bd = aox[1]|aox[3];
		gh = g|h;
                printf("     end-end distances : ab=%f, cd=%f, ac=%f, bd=%f, gh=%f\n", ab,cd,ac,bd,gh);
                printf("     end-box distances : ga=%f, gb=%f, hc=%f, hd=%f\n", ga,gb,hc,hd);
                ga = aox[0]|g; gb = aox[1]|g;
                hc = aox[0]|h; hd = aox[3]|h;
                printf("     end-box distances : ga=%f, gb=%f, hc=%f, hd=%f\n", ga,gb,hc,hd);
		if(perp) printf("perpendicular\n");
		if(parr) printf("parallel\n");
		KILL
...
float recp_area (Vec *box, float ideal, float tail)
{ // measures the reciprocal area between box0-1 and box2-3 (edges must have equal end separations)
Vec	a,b,x,y;
float	ab,cd, sum, step = 0.1;
int	i, n;
	x = (box[1]-box[0]).getNorm();
	y = (box[3]-box[2]).getNorm();
	if (x*y < -0.1) { printf("*NB* box edges antiparallel (x.y=%f)\n",x*y); KILL }
	ab = box[0] | box[1];
	cd = box[2] | box[3];
	if (fabs(ab-cd) > 0.05) { printf("*NB* box has diff edges\n"); KILL }
	n = (int)(ab/step);
	sum = 0.0;
	x *= step; y *= step;
	a = box[0]; b = box[2];
	...
	{ float d = (a|b)-ideal;
		if (d < 0.0) sum += 1.0;
			else sum += exp(-d*d*tail);
		a += x; b += y;
	}
	return sum*step;
}

float recp_area (Seg a, Seg b, float ideal, float tail)
{ // measures the reciprocal area between line segments a and b
Vec	box[4];
float	over = lineOline(a,b,box);
	if (over < NOISE) return 0.0;
	return recp_area(box,ideal,tail);
}
